...
#include "FabServer/UmgMcpJsonCompat.h"
...
namespace
{
	...
	FString BluecodeVarTypeFamily(const UWidgetBlueprint* Blueprint, const FString& VarName)
	{
		if (!Blueprint)
		{
			return TEXT("");
	...
	FString BluecodeMathTypeFamily(const UWidgetBlueprint* Blueprint, const FString& TargetName, const FString& Left, const FString& Right)
	{
...
FString UUmgBlueprintFunctionSubsystem::ExecuteGraphAction(UWidgetBlueprint* WidgetBlueprint, const TSharedPtr<FJsonObject>& Payload)
{
    ...
    {
        TSharedPtr<FJsonObject> Result = GetEvents(WidgetBlueprint, Payload);
        if (Result.IsValid())
        {
    ...
    {
        Result = SearchNodes(TargetGraph, WidgetBlueprint, Payload);
    }
    ...
    {
        Result = ApplyBluecode(TargetGraph, WidgetBlueprint, Payload);
    }
    else if (SubAction == TEXT("bluecode_apply_variables"))
    {
        Result = ApplyBluecodeVariables(WidgetBlueprint, Payload);
    }
    else if (SubAction == TEXT("bluecode_delete"))
    {
        Result = DeleteBluecode(WidgetBlueprint, TargetGraph, Payload);
    }
...
TSharedPtr<FJsonObject> UUmgBlueprintFunctionSubsystem::CreateNodeInstance(UEdGraph* TargetGraph, const TSharedPtr<FJsonObject>& Payload, UEdGraphNode*& OutNode)
{
    ...
    auto SetupNode = [&](UEdGraphNode* Node) {
        ...
        auto TryWireExpressionNode = [&](const FString& Expression, UEdGraphPin* TargetPin, int32 LayoutIndex, FString& OutError)
        {
            ...
            {
                FString Family = BluecodeMathTypeFamily(Cast<UWidgetBlueprint>(TargetBlueprint), TEXT(""), BinaryLeft, BinaryRight);
...
TSharedPtr<FJsonObject> UUmgBlueprintFunctionSubsystem::GetEvents(UWidgetBlueprint* Blueprint, const TSharedPtr<FJsonObject>& Params)
{
    TSharedPtr<FJsonObject> Result = MakeShared<FJsonObject>();
    ...
    if (!ScopeWidgetName.IsEmpty() && Blueprint->WidgetTree)
    {
        ScopeWidget = Blueprint->WidgetTree->FindWidget(FName(*ScopeWidgetName));
        if (!ScopeWidget)
        {
...
TSharedPtr<FJsonObject> UUmgBlueprintFunctionSubsystem::ApplyBluecode(UEdGraph* Graph, UWidgetBlueprint* Blueprint, const TSharedPtr<FJsonObject>& Params)
{
...
TSharedPtr<FJsonObject> UUmgBlueprintFunctionSubsystem::ApplyBluecodeVariables(UWidgetBlueprint* Blueprint, const TSharedPtr<FJsonObject>& Params)
{
...
TSharedPtr<FJsonObject> UUmgBlueprintFunctionSubsystem::DeleteBluecode(UWidgetBlueprint* Blueprint, UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
...
TSharedPtr<FJsonObject> UUmgBlueprintFunctionSubsystem::SearchNodes(UEdGraph* Graph, UWidgetBlueprint* Blueprint, const TSharedPtr<FJsonObject>& Params)
{
    FString Query;
